...
CREATE TABLE IF NOT EXISTS legal_tickets (id BIGINT PRIMARY KEY AUTO_INCREMENT, user_id BIGINT, assignee_id BIGINT, type VARCHAR(32), description TEXT, status VARCHAR(32), attachments JSON);
CREATE TABLE IF NOT EXISTS ticket_transfers (id BIGINT PRIMARY KEY AUTO_INCREMENT, ticket_id BIGINT NOT NULL, from_assignee_id BIGINT, to_assignee_id BIGINT NOT NULL, status VARCHAR(32), created_at DATETIME, accepted_at DATETIME);
